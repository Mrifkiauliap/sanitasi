<x-app-layout>
    <div class="space-y-6">
        <form method="POST" action="{{ route('pengaturan.manajemen-user.update', $user->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <x-content-card title="Edit User" subtitle="Edit data user" icon="edit">
                <x-slot name="action">
                ...
                <div class="space-y-6">
                    ...
                    @if($user->photo_path)
                        <div class="flex justify-center">
                            <img src="{{ Storage::url($user->photo_path) }}" alt="{{ $user->name }}" class="h-32 w-32 rounded-full object-cover border-4 border-gray-100 shadow-sm">
                        </div>
                    @endif
                    ...
                    <!-- Foto Profile -->
                    <div>
                        <x-input-label for="image" :value="__('Ganti Foto Profil')" />
                        ...
                        <p class="mt-1 text-sm text-gray-500">Kososngkan jika tidak ingin mengubah foto. PNG, JPG, JPEG, WEBP (Max. 2MB)</p>
